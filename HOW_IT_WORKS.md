...
Running `commit-roaster setup` handles everything automatically:

1. **Detects your shell** — checks the `$SHELL` environment variable for Bash, Zsh, or Fish. On Windows (when `$SHELL` is not set), defaults to PowerShell.
2. **Finds the correct profile file** — `~/.zshrc`, `~/.bashrc`, `~/.config/fish/config.fish`, or PowerShell's `$PROFILE`.
3. **Appends the integration line** — adds it with a `# commit-roaster shell integration` marker so it can be cleanly removed later.
4. **Is idempotent** — running `setup` multiple times won't duplicate the entry.
5. **Cleans up legacy hooks** — if you used an older version that modified `.git/hooks`, it automatically removes those artifacts.

To reverse the process, `commit-roaster teardown` surgically removes only the two lines it added, leaving the rest of your shell profile untouched.
...
Equivalent wrappers are generated for **Fish** (using `function git` / `$status`) and **PowerShell** (using `& git.exe @args` / `$LASTEXITCODE`).
...
5. **Cross-Platform**: Works on macOS, Linux, and Windows with native shell syntax for each platform.
